...
import { View, Text, TouchableOpacity, Image, Modal, TextInput, Button, Alert } from "react-native";
...
import { getUser, changeAvatar, updateStepsCount, changeStepsGoal } from "../../services/api";
...
function User({ navigation }) {
    ...
    navigation.setOptions({
      ...
      headerRight: () => (
        <TouchableOpacity onPress={() => navigation.navigate('Settings')}>
          <MaterialIcons name="settings" size={28} color="white" style={{ marginRight: 15 }} />
        </TouchableOpacity>
      ),
  ...
  const progress = user ? (initialStepCount + sessionSteps) / user.stepsGoal : 0;
...
<MaterialCommunityIcons name="foot-print" size={30} color={colors.primary} style={UserStyles.stepIcon} />
        <TouchableOpacity onPress={handleOpenSetGoalModal} style={{ minHeight: 40, justifyContent: 'center' }}>
          <Progress.Bar
            progress={progress}
            width={150}
            height={20}
  ...
  {`${initialStepCount + sessionSteps}/${user?.stepsGoal || 'Nie ustawiono celu'} steps`}
</Text>
      </View>
